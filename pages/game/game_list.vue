<template>
	<view class="bg">
		...
		<view class="list">
			<view class="info" v-for="(d, i) in gameList" :key="i" @click="getGame(d.status, d.url+'?gid='+d.id, d.target)">
				<image class="info-img" :src="d.img" mode="aspectFill"></image>
				<view class="list-content">
					<view class="list-title">{{d.title}}</view>
					<view class="list-time" v-if="d.status==1">{{utils.timeTodate('Y-m-d',d.start_time)}} 至 {{utils.timeTodate('Y-m-d',d.end_time)}}</view>
					<view class="list-time" style="color:#DD524D;" v-else>{{utils.timeTodate('Y-m-d',d.start_time)}} 至 {{utils.timeTodate('Y-m-d',d.end_time)}}</view>
				</view>
			</view>
		</view>
	...
	methods: {
		...
		getGame(e, url, target) {
			...
			if(target){
				...
				//plus.runtime.openURL('http://www.baid.com');
				_self.skipUrl('/pages/member/activity?type='+url, 2)
				//console.log(url)
			}
		},
...
.list-content view{ display:inline-block; }
.list-time{ float:right; }
.info-img{ width:100%;height:185px;border-radius:10px;float:left; }
.content-head{ 
	position: fixed;
	width: 100%;
	height: 30px;
...
.list{ margin-top:33upx;padding-bottom:15px; }
</style>
