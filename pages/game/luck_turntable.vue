	...
	<view class="conbox">
		<view class="container">
						...
						<view class="canvas-list">
							<view class="canvas-item" :style="[{zIndex:index2}]" v-for="(iteml,index2) in awardsList" :key="index2">
								<view class="canvas-item-text" :style="[{transform:'rotate('+iteml.turn+')'}]">
									...
									<text style="font-size:12px;margin-top:5px;" v-if="iteml.amount>0">+{{iteml.amount}}</text>
								</view>
							</view>
						</view>
					</view>
			...
			<view class="typecheckbox">
				<view :class="mold==i?'left active':'left'" @click="choosetype(i)" v-for="(t, i) in awardsConfig.luckMold" :key="i">
					{{t}}
				</view>
			</view>
			<!-- 规则 -->
				...
				<view class="myrewards" @click.stop="openshadow">
					...
					<view class="itembox">
						<view class="item" v-for="(items,i) in myPrizelist" :key="i">
							<view class="left">
								<image :src="items.img" style="width:14px;height:14px;float:left;margin-right:10px;margin-top:-1px;"></image>
			...
			<view class="winning" v-if="isWinning" @click="isWinningFun">
				...
				<view class="winning-content">
					...
					<view class="winning-title">
						<view style="font-size:33upx;width:100%;text-align:center;line-height:30px;">{{winningTitle}}</view>
...
export default {
	...
	methods: {
		...
		getmyPrize(){
			...
			var that = this
			uni.request({
				url: api.apiData.getPrize+'/gameId/'+that.gameId+'/userId/'+that.userId+'/type/3/page/1/num/500',
				method: 'GET',
				success: (e) => {
					uni.hideLoading();
					if(e.data.Code == 201){
		...
		initdata(that) {
			uni.request({
				url: api.apiData.getPrize+'/gameId/'+that.gameId+'/userId/'+that.userId+'/type/1/mold/'+that.mold,
				method: 'GET',
				success: (e) => {
					uni.hideLoading();
					if(e.data.Code == 201){
		...
		// 发起抽奖  
		playReward() {
			uni.showLoading({ title:'正在抽奖...',mask:true });
			var that = this
			...
			// 获取奖品
			uni.request({
				url: api.apiData.getPrize+'/gameId/'+that.gameId+'/userId/'+that.userId+'/type/2/mold/'+that.mold,
				method: 'GET',
				header: {
...
<style scoped>
.winning{ position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:999; }
.winning-content{ width:100%;height:580upx;top:45%;position:absolute;margin-top:-275upx;text-align:center;color:#b01a1c;font-weight:bold; }
.winning-title{ position:absolute;top:110upx;left:50%;width:180upx;font-size:25upx;margin-left:-85upx;text-align:left;line-height:40upx; }
.myLuck {
	position: relative;
	left: 20upx;
	top: -10upx;
