	...
	<view class="bg">
		...
		<view class="head-cono">
			<view class="head-cont">
				...
				<view class="head-cont-bank">到账银行卡</view>
				<view class="head-cont-bankList" v-if="userInfo.bankId > 0" @click="skipUrl('/pages/member/bank_list', 2)">{{userInfo.bankName}} (**** **** **** {{userInfo.bankSn}})</view>
				<view class="head-cont-bankList color" v-else @click="skipUrl('/pages/member/add_bank', 2)">请先添加到账银行卡</view>
		...
		<view class="content" id="toTop" :style="{ height: contentHeight + 'px' }">
			<input @input="onKeyInput" type="number" class="money" placeholder="请输入提现余额数" placeholder-style="color:#ffffff;">
...
.money{ height: 30px;line-height: 30px;margin: 0 auto 15px;display: inline-block;width: 50%;border-bottom: 1px solid #CCCCCC; }
.head-cont-wz{ padding: 30px 0 10px; }
.head-cont-money{ font-size: 50px;color: #ff5e63;font-weight: bold; }
.head-cont-bank{ padding: 10px 0; }
.head-cont-bankList{ border-radius: 5px;border: 1px solid #E2E2E2;width: 90%;margin: auto;font-size: 15px;line-height: 30px; }
.bg{ background: #e14351; }
.head-bg{
...
.content view { 
	width: 70%;background: #eeff37;margin: 0 auto 15px;color: #666666;
