			...
			<view class="watch-content" v-for="(v, i) in logList" :key="i" @click="playVideo(v.videoId)">
				...
				<view class="video-info">
					...
					<view class="video-class">
						<text style="background:#D89720;padding:0 5px;color:#FFFFFF;font-size:11px;border-radius:3px;" v-if="tab!=1">{{v.playTime}}</text>
					...
					<view class="video-time">
						<view v-if="tab==1">
							<text style="background:#D89720;padding:0 5px;color:#FFFFFF;font-size:11px;border-radius:3px;">{{v.playTime}}</text>
						...
						<view v-else-if="tab==2">
							<view style="color:#ff962f;width:40%;margin-top:2px;">
								<image src="/static/imgs/like.png" style="width:13px;height:13px;float:left;margin:3px 3px 0 0;"></image>
								{{v.likes}}
							</view>
...
.watch-radius{ width:18px;height:18px;background:#FF8F00;float:left;border-radius:50%;margin-left:-11px;margin-top:30px; }
.video-class{ font-size:13px; }
...
.video-info view{ width:100%;float:left;color:#999; }
.video-img image{ width:100%;height:100%;border-radius:10px;box-shadow: 0px 4px 6px 0px rgba(0, 0, 0, 0.5); }
.video-img{ width:35%;padding:0 0 0 2%;height:80px; }
.video-info{ width:58%;float:right; }
.watch-content view{ display:inline-block; }
.watch-content{ width:90%;margin:0 5%;border-left:4px solid #FF8F00;padding:5px 0 0 0; }
.watch-list{ width:100%;padding-bottom:15px; }
.select{ border-bottom:2px solid #FFFFFF;padding-bottom:5px; }
.content-head{ position:fixed;width:100%;height:30px;line-height:30px;top:27px;text-align:center;color:#FFFFFF;z-index:10; }
.watch-head{ 
	width:100%;
	text-align: center;
...
.watch-head view{ display:inline-block;width:60px;font-size:16px;color:#FFFFFF; }
.del-log{ background:rgba(0,0,0,0.5);padding:5px 2px 5px 8px;border-radius:50px 0 0 50px;position:absolute;right:0; }
</style>
