<template>
	...
	<view class="content">
		...
		<view class="mask" v-if="loading" style="background-color:#20202C;margin-top:0!important;z-index:9!important;" :style="{'margin-top':h5Top+'upx'}">
			...
			<view class="content-item">
				...
				<view class="item-line" v-if="classList.length">
					...
					<scroll-view scroll-x="true" :show-scrollbar="false" class="class-list">
						<view :class="uploadParam.cid==c.id?'class-item sel':'class-item'" v-for="(c, i) in classList" :key="i" @click="setCid(c.id)">{{c.name}}</view>
					</scroll-view>
				</view>
				...
				uni.chooseVideo({
					...
					success: res => {
						...
						if (_self.maxVideoSize > 0) {
							...
							if (size > _self.maxVideoSize) return api.showToast('视频大小不得超过'+_self.maxVideoSize+'M', 2000);
						}
						_self.files[1] = file;
							...
							complete: () => {
								...
								if (isMember) {
									...
									uni.setStorage({
									    ...
									    data: true,
									    success: () => {
									        api.jumpUrl('/pages/jump/index?t=1&u='+r.Data.uploadUrl, 'new');
									    }
									});
								} 
							}
