<template>
	<view class="bg"> 
		<view class="video-list">
			...
			<view 
				:class="v.downstatus?'video-info ok':'video-info no'" 
				v-for="(v, i) in downList" :key="i"
				v-if="v.delstatus">
				<view class="img" @click="playVideo(v.downstatus, v.title, v.url, v.poster)">
					<image class="pic" :src="v.poster" mode="aspectFill"></image>
					<image class="play" :style="{'margin-top':topUpx+'upx'}" src="@/static/imgs/play.png" mode="aspectFill" v-if="v.downstatus"></image>
				</view>
				<view class="video-content">
					<image class="bgx" src="@/static/imgs/delx.png" mode="aspectFill" @click="delVideo(v.videoid,i,v.url,v.downstatus)"></image>
					<view class="video-title">{{v.title}}</view>
					<view v-if="v.downstatus">
						<view class="video-kb">
							<view style="margin-left:2px;">已下载</view>
							<view style="float:right;margin-right:1px;">{{v.videosize}}<text v-if="v.type!='m3u8'">M</text></view>
						</view>
					...
					<view v-else>
						<view class="video-kb">
							...
							<view style="float:right;margin-right:2px;">
								{{v.downsize}}<text v-if="v.type!='m3u8'">M</text>/{{v.videosize}}<text v-if="v.type!='m3u8'">M</text>
							</view>
						</view>
						<view class="video-lenght">
...
export default {
	...
	methods: {
		downVideo(t){
			/* ======添加视频下载任务示例=====s */
			// 获取文件大小 http://ms.cqpig.cn/24.mp4 
			/* uni.request({
			...
			var data = {
				...
				videoId: rand,
				title: '测试视频'+rand,
				url: t=='mp4'?'http://ms.cqpig.cn/24.mp4':'https://vip4.ddyunbo.com/20210607/wY6rIqTw/index.m3u8',
