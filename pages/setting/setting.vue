<template>
	<view>
		...
		<view class="head-tx" @click="detectionApp">
			<view>检测更新 <view class="newTig" v-if="isNew"></view></view>
			<view class="head-acc"> {{newVersion}} <image src="/static/imgs/r.png" style="width:12px;height:10px;"></image></view>
		</view>
		
		<!-- #ifdef APP-PLUS -->
		<view class="head-tx" @click="clearCache">
			<view>清理缓存</view>
			<view class="head-acc">{{cacheSize}} <image src="/static/imgs/r.png" style="width:12px;height:10px;"></image></view>
		</view>
		<!-- #endif -->
		
		<view class="head-tx" @click="skipUrl('/pages/member/activity?type=agreement', 2)">
			<view>服务协议</view>
			<view class="head-acc"> <image src="/static/imgs/r.png" style="width:12px;height:10px;"></image></view>
		</view>
...
<style>
	...
	.update-center{ margin: 14px 5%;font-size:16px;color:#666666; }
	.update-botton{ 
		flex:1;
		height:40px;
	...
	.update-ing{ height:14px;width:90%;border:1px solid #E5E5E5;margin:auto;text-align:center;border-radius:14px; }
	.update-ing .ing{ height:14px;background:#ff6f33;border-radius:14px;font-size:10px;color:#FFFFFF;line-height:14px; }
	.update-wz{ width:100%;text-align:center;margin-top:10px;font-size:12px;color:#ff6f33; }
	.update{ width:100%;height:100%;background:rgba(0,0,0,0.5);position:fixed;top:0; }
	.update-content{ 
		width: 70%;
		background: #FFFFFF;
		//height: 225px;
	...
	.newTig{ width:12px;height:12px;background:#F9221D;border-radius:50%; }
	.input{
		width: 96%;
		padding: 5px 2%;
