<template>
	<view class="conbox">
		...
		<view class="top-box">
			...
			<view class='video-line-box' v-if="_self.videoline.length > 1 && loadings != 1" >
				<view class='video-line' @click="changeline(0, _self.videoline[0].title)">
					<text style="font-size: 14px;" :class="_self.currentline == 0 ?'current-line':'no-current-line'">{{ _self.videoline[0].title}} </text>
				</view>
				...
				<view class='video-line' @click="changeline(1, _self.videoline[1].title)">
					<text style="font-size: 14px;" :class="_self.currentline == 1 ?'current-line':'no-current-line'">{{ _self.videoline[1].title}}</text>
				</view>
			</view>
			<image class="before-ad" v-if="isHide == 0" @click="open(ad_url)" :src="ad_img" mode="aspectFit"></image>
			<image class="pause-ad" :style="{ width:windowWidth+ 'px',height: pauseHeight + 'px',left:pauseLeft+ 'px',top:pauseTop+'px', }"   v-if="ispause == 1" @click="open(ad_url)" :src="ad_img"></image>
			...
			<view class="rate-list" v-if="isShowRate">
				<text class="rate-list-text"
					:class="nowRate==r?'rate-list-text-sel':''"
					@click="setRate(r)"
		...
		<view class="video-gold-tips-box-mask" v-if="_self.showNotGoldTips && loadings != 1"  >
			<view class="video-tips-box video-gold-tips-box" v-if="_self.showNotGoldTips" >
				<view style="color: #fff;font-size: 16px;">{{ _self.showMsgContent }}</view>
				<view class="tips-bt-box">
					<view class="tips-bt flex-row" @click="clickConfirm('_self.'+_self.confirmType+'()')">{{ _self.confirmText }}</view>
					<view class="tips-bt flex-row-rtl" @click="clickCancelBt('_self.'+_self.cancelType+'()')">{{ _self.cancelText }}</view>
				</view>
			</view>
		</view>
		
		<view class="content-head" @click="back(1)">
			<image class="backimage" src="@/static/imgs/back.png" mode="aspectFill" style="width:20px;height:20px;margin:5px 0 0 -3px;"></image>
		</view>
		
		<view class='placeholder-box' style="width:100%;height:270px;background-color: #1D1D28;" v-if="_self.videoline.length > 1"></view>
		<view class='placeholder-box' style="width:100%;height:225px;background-color: #1D1D28;" v-else></view>
		
		<div class="ad-list" @click="jumpAdUrl('/pages/member/poster', 'new')" v-if="!info.isVip&&isShowFree&&freeWacth.count>0">
			<text class="watch-free" :style="{'background-image':'url('+imageURL+')'}">今日剩余观看次数 {{freeWacth.free}} / {{freeWacth.count}}</text>
		</div>
		...
		<view class='video-content' v-if="loadings != 1">
			<view class='video-info-box'>
				...
				<div class='flex-row relative'>
					...
					<div class="video-gold" style="padding-right:0;margin-left:0;" v-else-if="_self.info.videoInfo.gold>0">
						<image class="video-gold-image" src="@/static/imgs/gold.png" style="height:13px;width:13px;"></image>
						<text class='video-gold-num' style="color:#ff9030;font-size:12px;">{{_self.info.videoInfo.gold}}</text>
					</div>
					...
					<image src="@/static/imgs/_rd.png" style="height:13px;width:13px;margin-left:5px;margin-bottom:-2px;"></image>
					<text class='video-click-num' style="font-size:12px;color:#999999;">{{_self.info.videoInfo.click}} 次播放</text>
					<image src="@/static/imgs/_rd.png" style="height:13px;width:13px;margin-left:5px;margin-bottom:-2px;"></image>
					<text class='video-click-num' style="font-size:12px;color:#999999;">{{_self.info.videoInfo.className}}</text>
					<image src="@/static/imgs/_rd.png" style="height:13px;width:13px;margin-left:5px;margin-bottom:-2px;"></image>
					<text class='video-click-num' style="font-size:12px;color:#999999;">{{utils.timeTodate('Y年m月d日',info.videoInfo.add_time+'')}}</text>
				...
				<div class='video-operation-box flex-row relative' style="margin-bottom:5px;margin-top:0px;width:100%;">
					<div style="display:inline-block;height:80upx;line-height:80upx;margin:0 20upx 0 -20upx;" @click="like()">
						<image class="video-operation-image" v-if="info.isLike == 1" src="@/static/imgs/like.png"></image>
					...
					<div style="display:inline-block;height:80upx;line-height:80upx;" @click="collection()">
						<image class="video-operation-image" v-if="info.isCollection == 1" src="@/static/imgs/collection.png"></image>
					...
					<div class="show-introduction flex-row" style="width:150upx;text-align:right;line-height:60upx;height:60upx;">
						<text class="show-introduction-text" @click="showIntroduction()">简介</text>
						<uni-icons class="show-introduction-icons" type="forward" color="#cccccc" size="22" style="margin-right:-5px;" @click="showIntroduction()"></uni-icons>
					</div>
				</div>

			</view>
			
			<div style="background-color:#1D1D28;padding-left:20rpx;height:165upx;" v-if="hDvdList.length">
				<div class="flex-row flex-start">
					<text class="title" style="color:#FFFFFF;font-size:16px;">选集(共{{hDvdList.length}}集){{setLineName}}</text>
				</div>
				<div style="width:100%;height:87upx;">
					<scroll-view class="teleplay-scroll" :show-scrollbar="false" :scroll-x="true" style="padding-right:35rpx;">
						<div style="display: inline-block;margin-top: 20upx;" v-for="(h, i) in hDvdList" :key="i" @click="selectedVideo(h)">
			...
			<div style="height:80upx;line-height:80upx;background-color:#20202C;margin-top:20upx;padding:0 20upx;" @click="goAuthorHome(author.id)" v-if="author.id>0">
			...
			<div style="width:100%;background-color:#1D1D28;padding:10upx 0 20upx 0;" v-if="adList.length>0">
				<div class="ad-list" v-for="(a, i) in adList" :key="i">
					<image class="ad-img" :style="{ height: a.height+'px' }" :src="a.content" mode="aspectFill" @click="jumpAdUrl(a.url, 'web')"></image>
				</div>
			</div>
			...
			<view class="guess-video-list flex-row">
				<view style="width: 100%;height:21px;line-height:21px;padding: 0 20rpx;font-size:16px;color:#FFFFFF;margin-left:10px;margin-bottom:30upx;border-left: 6px solid #FF8F00;">
				...
				<view class="video-info" v-for="(i, k) in info.guess" :key="k" @click="playVideo(i.id)">
					<view>
						...
						<view class="video-title">{{i.title}}</view>
						<view class="video-gold"><image src="@/static/homePage/click.png" mode="aspectFill"></image>{{i.click}}</view>
			...
			<view class="uni-comment-list-box">
				<view style="width:100%;height:21px;line-height:21px;font-size:16px;color:#FFFFFF;">
					<view style="display:inline-block;position:absolute;border-left: 6px solid #FF8F00;padding-left:10px;">网友吐槽</view>
				</view>
					...
					<view class="uni-comment-list flex-row" v-for="(i, k) in commentList.list" :key="k" >
						...
						<view class="uni-comment-body flex-row">
							...
							<view class="uni-comment-date">
								<view class="uni-comment-addtime" v-if="i.last_time">{{utils.timeTodate('Y年m月d日', i.last_time+'')}}</view>
			...
			<div class="add-comment-box" :style="{ bottom: keyheight+'px' }" >
				<input class="comment-input" v-model="commentContent" type="text" placeholder="我要说点什么~" confirm-type="send" @confirm="comment(e)" @input="onInput" adjust-position="false" @keyboardheightchange="keyboardheightchange">
		...
		<uni-popup ref="showintroduction" type="bottom" @change="change">
			<div class="introduction-box" :style="{'min-height': (_self.SystemInfo.screenHeight-225) + 'px', 'max-height': (_self.SystemInfo.screenHeight-225) + 'px',}">
				<div class="introduction-head">
					<text class="introduction-title">视频简介</text>
					<image class="introduction-close" src="@/static/imgs/delx.png" @click="cancel('introduction')"></image>
				</div>
				<div style="margin:15px;border-bottom:1px solid #F5F5F5;padding-bottom:20upx;">
					...
					<div style="flex-direction:row">
						...
						<text class="introduction-content">{{utils.timeTodate('Y年m月d日', info.videoInfo.add_time+'')}}</text>
					</div>
				</div>
				<div v-if="info.videoInfo.info">
					<jyf-parser :html="info.videoInfo.info" ref="article" style="font-size: 16px;padding: 10px;">数据正在加载中...</jyf-parser>
				</div>
				<div v-else>
		...
		<view class="load-img" :style="{'height':SystemInfo.windowHeight+'px'}" @touchmove.stop.prevent="moveHandle" v-if="showLoading">
			<image src="/static/load.gif" mode="aspectFit" style="width:500rpx;height:400rpx;"></image>
		</view>
	</view>
...
<style>
	...
	.introduction-name, .introduction-content{
		font-size: 14px;
		color: #444;
		line-height: 22px;
		width: 110upx;
	...
	methods: {
		...
		selectedVideo(e, i='') {
			if (userid) {
				...
				if (!_self.info.isVip) {
					if (!_self.info.alreadyBuy&&_self.info.videoInfo.gold>0) return api.showToast('付费视频，请先购买视频', 2000);
				}
						...
						uni.setStorage({
							...
							complete: () => {
								if (_self.initialtime>0) {
									api.showToast('已恢复至'+_self.formatSeconds(_self.initialtime)+'秒开始观看', 2000);
								} else {
									uni.hideLoading();	
								}
							}
		...
		startVideo(e) {
		    ...
		    if (_self.info.videoInfo.gold > 0) {
		    	...
		    	if (userid) {
					...
					_self.showMsgContent = _self.info.watch>0?'游客可试看'+_self.info.feeLook+'s，注册可免费观看'+_self.info.watch+'部视频':'游客可试看'+_self.info.feeLook+'秒，登录后无限制';
					_self.showNotGoldTips = true;
			...
			uni.request({
				...
				success: (res)=> {
					...
					if (_self.info.isHDvd==0) {
						...
						if (_self.number==0&&_self.hDvdList.length) _self.number = _self.hDvdList[0]['id'];
					} else {
						...
						_self.bigLineId = 0;
						_self.subLineId = 0;
						if (_self.number==0&&_self.hDvdList.length) _self.number = _self.info.hDvd[0]['list']['id'];
					}
					_self.loadings = 0;
					let timer = setTimeout(() => {
						_self.startVideo();
		...
		timeupdate(e) {
			...
			if (_self.isTry == 1) {
				if (_self.trySeconds != 0) {
					_self.trySeconds = _self.info.feeLook - _self.formatSeconds(e.detail.currentTime);
				}
				if (e.detail.currentTime > _self.info.feeLook && _self.isTryEnd != 1) {
					_self.isTryEnd = 1;
					_self.stop();
					//试看结束
					if (userid) {
						...
						_self.showMsgContent = '试看结束，升级VIP无限观看哦';
						_self.showNotGoldTips = true;
						...
						_self.showMsgContent = _self.info.watch>0?'试看结束，注册账号可免费观看'+_self.info.watch+'部视频':'视频已试看结束，请登录或重新试看';
						_self.showNotGoldTips = true;
		...
		download(e) {
			if (userid) {
				if (_self.info.download != 1) {
					api.showToast('该视频只有VIP才能下载', 2000);
				} else {
					window.open(_self.info.videoInfo.download_url);
				}
			} else {
				uni.showModal({
					content: '要登录后才可以下载',
